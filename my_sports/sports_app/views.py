...
from .models import Enquiry, AddRegister, Duration, coachReg, Coach_allocation, KitItem, Item_entry, kit_distribution, \
    ...
    Attendence, Coach_Attendence,StockAdjustment
...
def product_dist(request):
    if request.method == 'POST':
        ...
        # Get the product details from the hidden input field
        products_data = request.POST.get('products')  # This should be a JSON string

        # Parse the JSON data
        try:
            products = json.loads(products_data)  # Safely parse the JSON string into a Python list
        except json.JSONDecodeError:
            products = []  # In case of an error, set products to an empty list
            print("Error decoding JSON.")

        # Check if products exist
        if not products:
            print("No products added.")
            # Handle the case where no products are added, for example, return an error message
...
def save_sale(request):
    if request.method == 'POST':
        try:
            ...
            # Save player and product details
            for product in products:
                sale = Sale(
...
def stock_adjustment(request):
    if request.method == 'POST':
        ...
        if product_name and description and quantity.isdigit():
            ...
            StockAdjustment.objects.create(product_name=product_name, description=description, quantity=quantity)
